...
{
	static sample_t clev[4] =
	    { LEVEL_3DB, LEVEL_45DB, LEVEL_6DB, LEVEL_45DB };
		      ...
		      sample_t level, int dither, int end)
{
	...
	for (i = 0; i < end; i++) {
		...
		case 3:
			coeff[i] =
			    q_3[bitstream_get(state, 3)] * factor[exp[i]];
			continue;
		...
		case 4:
			coeff[i] =
			    q_5[bitstream_get(state, 4)] * factor[exp[i]];
			continue;

		default:
			coeff[i] =
			    ((bitstream_get_2(state, bapi) << (16 - bapi)) *
			     factor[exp[i]]);
		}
	}
}
...
int a52_block(a52_state_t * state)
{
	...
	if (lfeexpstr != EXP_REUSE) {
		...
		if (parse_exponents
		    (state, lfeexpstr, 2, state->lfe_expbap.exp[0],
	...
	if (do_bit_alloc) {
		if (zero_snr_offsets(nfchans, state)) {
			memset(state->cpl_expbap.bap, 0,
			       sizeof(state->cpl_expbap.bap));
			for (i = 0; i < nfchans; i++)
				memset(state->fbw_expbap[i].bap, 0,
				       sizeof(state->fbw_expbap[i].bap));
			memset(state->lfe_expbap.bap, 0,
			       sizeof(state->lfe_expbap.bap));
		} else {
			if (state->chincpl && (do_bit_alloc & 64))	/* cplinu */
				a52_bit_allocate(state, &state->cplba,
						 state->cplstrtbnd,
						 state->cplstrtmant,
						 state->cplendmant,
						 state->cplfleak << 8,
			...
			for (i = 0; i < nfchans; i++)
				if (do_bit_alloc & (1 << i))
					a52_bit_allocate(state, state->ba + i,
							 0, 0,
							 state->endmant[i], 0,
							 0,
							 state->fbw_expbap + i);
			if (state->lfeon && (do_bit_alloc & 32)) {
				...
				a52_bit_allocate(state, &state->lfeba, 0, 0, 7,
						 0, 0, &state->lfe_expbap);
			}
		}
	}
	...
	for (i = 0; i < nfchans; i++) {
		...
		coeff_get(state, samples + 256 * i, state->fbw_expbap + i,
	...
	if (state->lfeon) {
		if (state->output & A52_LFE) {
			coeff_get(state, samples - 256, &state->lfe_expbap,
			...
			coeff_get(state, samples + 1280, &state->lfe_expbap,
				  &quantizer, 0, 0, 7);
		}
	}
	...
	if (i < nfchans) {
		...
		for (i = 0; i < nfchans; i++) {
			...
			if (coeff[i]) {
				if (blksw[i])
					a52_imdct_256(samples + 256 * i,
						      samples + 1536 + 256 * i,
						      bias);
				else
					a52_imdct_512(samples + 256 * i,
						      samples + 1536 + 256 * i,
						      bias);
			} else {
				int j;
