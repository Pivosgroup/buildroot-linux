...
static int adts_aac_probe(AVProbeData *p)
{
    ...
    if (ff_id3v2_match(buf0)) {
	 id3_len = ff_id3v2_tag_len(buf0);
	 if(id3_len > (p->buf_size -4))
	 	return 0;
        buf0 += id3_len;
    }
...
{       
    if(buf[4]&0x80)             //byte4,bit7: copyright_id_present
    {
        if((buf[13]&0x10)==0)
        {
            st->codec->bit_rate = ((buf[13]&0xf)<<19)|(buf[14]<<11)|(buf[15]<<3)|(buf[16]>>5);
            //av_log(NULL, AV_LOG_INFO, "[adif_header_parse:%d]%02x %02x %02x %02x\n",__LINE__,buf[13],buf[14],buf[15],buf[16]);

        }
    }
    else if((buf[4]&0x10)==0)        //byte4,bit4 : bitstream_type 0:CBR 1:VBR
    {
        st->codec->bit_rate = ((buf[4]&0xf)<<19)|(buf[5]<<11)|(buf[6]<<3)|(buf[7]>>5);
        //av_log(NULL, AV_LOG_INFO, "[adif_header_parse:%d]%02x %02x %02x %02x\n",__LINE__,buf[4],buf[5],buf[6],buf[7]);
    }
