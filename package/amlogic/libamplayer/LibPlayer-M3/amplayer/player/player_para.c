...
int player_offset_init(play_para_t *p_para) 
{
    ...
    if (p_para->playctrl_info.time_point >= 0) {
        ...
        if (ret != PLAYER_SUCCESS) {
            ...
            log_error("[%s:%d]time_search to pos:%fs failed!", __FUNCTION__, __LINE__, p_para->playctrl_info.time_point);
            goto init_fail;
        }
