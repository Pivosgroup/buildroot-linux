...
{    
    ...
    if(ret == VALID_CMD)
    {
        ...
        else if(cmd->ctrl_cmd & CMD_PLAY || cmd->ctrl_cmd & CMD_PLAY_START)
        {
            ...
            if(player_register_update_callback(&t_para->play_ctrl_para.callback_fn,&update_statue,1000)!=PLAYER_SUCCESS)
...
void controler_run(global_ctrl_para_t *p)
{
    ...
    {      
		...
		{   
            ...
            if(cmd.ctrl_cmd!=0)
            {
                log_print("ctrl_cmd=%x ",cmd.ctrl_cmd);
            ...
            if(cmd.info_cmd!=0) log_print("info_cmd=%x ",cmd.info_cmd);
            if(cmd.set_mode!=0) log_print("set_mode=%x ",cmd.set_mode);            
            log_print("\n");
        ...
        for(i = 0; i < player_id.num; i++)
        {
            ...
            if(ret)
            {
                ...
                log_print("[controler_run:%d]bg=%d pid_num=%d exit pid=%d\n",__LINE__,p->background,player_id.num,pid);
            }
        }        
        if(p->background)
            continue;
