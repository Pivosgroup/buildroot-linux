 ...
 *
 * extern void fb_display(unsigned char *argbbuff,
...
void* convertRGB2FB(int fh, unsigned char *argbbuff, unsigned long count, int bpp, int *cpp);
...
void fb_display(void* arg,unsigned char *argbbuff, unsigned char * alpha, int x_size, int y_size, int x_pan, int y_pan, int x_offs, int y_offs)
{
    ...
    /* blit buffer 2 fb */
    fbbuff = convertRGB2FB(fh, argbbuff, x_size * y_size, var.bits_per_pixel, &bp);
...
void* convertRGB2FB(int fh, unsigned char *argbbuff, unsigned long count, int bpp, int *cpp)
{
	...
	case 8:
	    *cpp = 1;
	    ...
	    for(i = 0; i < count; i++)
		c_fbbuff[i] = make8color(argbbuff[i*3], argbbuff[i*3+1], argbbuff[i*3+2]);
	...
	case 15:
	    *cpp = 2;
	    ...
	    for(i = 0; i < count ; i++)
		s_fbbuff[i] = make15color(argbbuff[i*3], argbbuff[i*3+1], argbbuff[i*3+2]);
	...
	case 16:
	    *cpp = 2;
	    s_fbbuff = (unsigned short *) malloc(count * sizeof(unsigned short));
	    for(i = 0; i < count ; i++)
		s_fbbuff[i] = make16color(argbbuff[i*3], argbbuff[i*3+1], argbbuff[i*3+2]);
	    fbbuff = (void *) s_fbbuff;
	    break;
	case 24:
	case 32:
	    *cpp = 4;
	    ...
	    for(i = 0; i < count ; i++)
	    /*data squence bgra*/
		i_fbbuff[i] = ((argbbuff[i*4])& 0xFF)|         
			    ((argbbuff[i*4+1] << 8) & 0xFF00) |
			    ((argbbuff[i*4+2] << 16) & 0xFF0000)|
			    ((argbbuff[i*4+3] << 24) &0xff000000);
