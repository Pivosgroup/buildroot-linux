                            ...
                            int *data_size, AVPacket *avpkt)
{
    ...
    switch (ac->m4ac.object_type) {
        case AOT_AAC_MAIN:
        ac->avccontext->profile = FF_PROFILE_AAC_MAIN;
        break;
        case AOT_AAC_LC:
        ac->avccontext->profile = FF_PROFILE_AAC_LOW;
        break;
        default:
        ac->avccontext->profile = FF_PROFILE_UNKNOWN;
        break;
    }
    ...
    while ((elem_type = get_bits(&gb, 3)) != TYPE_END) {
        ...
        if (elem_type < TYPE_DSE) {
            if (!(che=get_che(ac, elem_type, elem_id))) {
                av_log(avccontext, AV_LOG_ERROR, "channel element %d.%d is not allocated\n",
                       elem_type, elem_id);
                return -1;
            }
            samples = 1024;
