...
static void mov_metadata_creation_time(AVMetadata **metadata, time_t time)
{
    ...
    if (time) {
        ...
        av_metadata_set(metadata, "creation_time", buffer);
    }
}
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 retry:
    ...
    if (st->discard != AVDISCARD_ALL) {
	  int64_t res;
	  res=url_fseek(sc->pb, sample->pos, SEEK_SET) ;
        if (res != sample->pos) {
            av_log(mov->fc, AV_LOG_ERROR, "stream %d, offset 0x%"PRIx64":res="PRIx64" partial file\n",
                   sc->ffindex, res,sample->pos);
            return -1;
        }
