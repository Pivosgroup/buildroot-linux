...
int match_ext(const char *filename, const char *extensions)//get file type, .vob,.mp4,.ts...
{
    const char *ext, *p;
    char ext1[32], *q;
                       ...
                       const char *headers)
{
    ...
    if (!fmt) {
        ...
        fmt = av_probe_input_format(pd, 0); //first time,pb is NULL
    }
        ...
        if (pktl) {
            ...
            if(   next_pkt->pts != AV_NOPTS_VALUE
                ...
                *pkt = *next_pkt;
                ...
                av_free(pktl);
				av_log(NULL,AV_LOG_INFO, "[%s:%d]eof=%d genpts=%d pkt->pts=0x%llx pkt->dtx=0x%llx",
									__FUNCTION__,__LINE__,eof,genpts,next_pkt->pts,next_pkt->dts);
                return 0;
            }
        }
        if(genpts){
            ...
            if(ret<0){
                if(pktl && ret != AVERROR(EAGAIN)){
                    eof=1;
					av_log(NULL,AV_LOG_INFO, "[%s:%d]ret=%d set eof",__FUNCTION__,__LINE__,ret);

                    continue;
                }else{
                	if(ret < 0)
						av_log(NULL,AV_LOG_INFO, "[%s:%d]ret=%d",__FUNCTION__,__LINE__,ret);
                              ...
                              int flags)
{
    ...
    b = nb_entries;
#if 0
    av_log(NULL, AV_LOG_INFO, "nb_entried %d, wanted ts 0x%llx\n", nb_entries, wanted_timestamp);
    if (st->index == 1)
    for (m=0; m<b; m++)
        av_log(NULL, AV_LOG_INFO, "chunk %d ts 0x%llx\n", m, entries[m].timestamp);
#endif
...
{	
	...
	for(;;)
	{
		...
		{		
        	//av_log(NULL, AV_LOG_INFO, "[%s:%d]first block buf1<>0\n",__FUNCTION__,__LINE__);
			memset(buf1,0,CHECK_FULL_ZERO_SIZE);
			url_fseek(ic->pb, offset + CHECK_FULL_ZERO_SIZE, SEEK_SET);
			do{
	            ret = get_buffer(ic->pb, buf1,CHECK_FULL_ZERO_SIZE);
	        }while(ret == AVERROR(EAGAIN));
	        if (ret < 0)
...
static void av_estimate_timings_from_pts(AVFormatContext *ic, int64_t old_offset)
{
   ...
   // unsigned int ori_nb_streams;   
		...
		if(offset > 0)
		{
			...
			{	
			    ...
			    for(;;) {
			        ...
			        read_size += pkt->size;
			        st = ic->streams[pkt->stream_index];
					av_log(NULL, AV_LOG_INFO, "[%s:%d]pkt->pts=0x%llx pkt->dts=0x%llx start_time=0x%llx\n",
							__FUNCTION__,__LINE__,pkt->pts,pkt->dts,start_time[pkt->stream_index]);
			        if (pkt->pts != AV_NOPTS_VALUE &&
			            start_time[pkt->stream_index] != AV_NOPTS_VALUE) {
			            end_time = pkt->pts;
			            duration = end_time - start_time[pkt->stream_index];
				...
				offset = offset - (DURATION_MAX_READ_SIZE<<retry);
			    if (offset < 0)
			        offset = 0;
				if(duration > 0)
					break;
    ...
    if (duration <= 0)
    {
        /* search again by frame */
        ori_nb_streams = ic->nb_streams;
        retry = 0;
        av_log(NULL, AV_LOG_INFO, "[%s] no duration detected\n", __FUNCTION__);
        while(filesize > (DURATION_MAX_READ_SIZE<<retry))
        {
            url_fseek(ic->pb, filesize  - (DURATION_MAX_READ_SIZE<<retry), SEEK_SET);
            for (;;){
                do{
                    ret = av_read_packet(ic, pkt);
                }while(ret == AVERROR(EAGAIN));
                ...
                /* only record the original streams */
                if(pkt->stream_index < ori_nb_streams)
                {
                    st = ic->streams[pkt->stream_index];
                    if (pkt->pts != AV_NOPTS_VALUE &&
                        start_time[pkt->stream_index] != AV_NOPTS_VALUE) {
    ...
    for(;;) {
        ...
        if (ret < 0) {
            ...
            for(i=0;i<ic->nb_streams;i++) {
                ...
                //add by xh,2010-08-26 for amlogic player,needn't find codec
                if(st)
                    ret = 0;
            }
            break;
...
void av_ioctrl(AVFormatContext *s,int cmd,unsigned long param1,int size)
{
    ByteIOContext *pb = s->iformat->flags & AVFMT_NOFILE ? NULL : s->pb;
    if(pb)
        url_ioctl(pb->opaque,cmd,param1,size);
}
...
static void dump_stream_format(AVFormatContext *ic, int i, int index, int is_output)
{
    ...
    av_log(NULL, AV_LOG_INFO, "    Stream #%d.%d [ Index: %d ]", index, i, i);
    /* the pid is an important information, so we display it */
    /* XXX: add a generic system */
    //if (flags & AVFMT_SHOW_IDS)
    av_log(NULL, AV_LOG_INFO, "[ id: %d ]", st->id);
    if (lang)
