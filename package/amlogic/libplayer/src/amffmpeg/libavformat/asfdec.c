...
static int asf_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    int video_stream_num = 0, audio_stream_num = 0;
    uint64_t avg_frame_time[128];
...
//av_log(s, AV_LOG_ERROR, "%d %d %d %d %d <%s>\n", i, stream_num, name_len, value_type, value_len, name);
            ...
            if (stream_num < 128)
           	...
           	avg_frame_time[stream_num] = get_le64(pb); // avg frametime in 100ns units
    ...
    for (;;) {
        ...
        if (asf->packet_size_left < FRAME_HEADER_SIZE
            ...
            if (asf->data_object_size != (uint64_t)-1 &&
                ((int64_t)(asf->packet_pos - asf->data_object_offset) >= (int64_t)asf->data_object_size))
            {
                av_log(NULL, AV_LOG_INFO, "[ff_asf_parse_packet] packet_pos %llu, object_size %llu, offset %llu\n",
                    asf->packet_pos, asf->data_object_size, asf->data_object_offset);
...
static void asf_build_simple_index(AVFormatContext *s, int stream_index)
{
    ...
    if (!guidcmp(&g, &index_guid)) {
        ...
        //add by X.H. need index more than 1
		if (ict > 1){
        	asf->index_read= 1;
		}
    }
